...
void gen_polaczenia(struct punkt vb,int indeks_ojca_slada){
    deb printf("siemanko jestem w (%d,%d) a ojciec_ind to %d\n",vb.i,vb.j,indeks_ojca_slada);
    visited[vb.i][vb.j] = 1;
...
int main(void){
    ...
    deb2{
        printf("Legowiska: \n");
        for(int i=0; i<licznik_l; i++) printf("legowiska[%d] = (%d,%d)\n",i,legowiska[i].i, legowiska[i].j);
        printf("\nŚladj: \n");
        for(int i=0; i<licznik_s; i++) printf("sladj[%d] = (%d,%d)\n",i,sladj[i].i,sladj[i].j);
        printf("\n");
    }
    ...
    deb2{
        printf("Polaczenia w grafie sladow bezposrednich (bez wymaganego przechodzenia przez slad):\n");
        for(int i=0; i<20; i++){
            for(int j=0; j<20; j++){
                if(graf_sladow[i][j]){
                    printf("mogę dojść z sladj[%d] do sladj[%d] tj. z (%d,%d) do (%d,%d)\n",i,j,sladj[i].i,sladj[i].j,sladj[j].i,sladj[j].j);
                }
            }
        }
        printf("\nCzy slad ma polaczenie bezposrednie z legoiwskiem:\n");
        for(int i=0; i<licznik_s; i++)
            printf("czy polaczony slad nr. %d: %d\n",i,polaczony_legowiskiem[i]);
        printf("\n");
    }
