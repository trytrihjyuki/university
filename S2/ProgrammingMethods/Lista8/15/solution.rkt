...
(struct thunk (e env) #:transparent)
...
(define (eval-env e env)
  (match e
    ...
    [(binop op l r) ((op->proc op) (eval-env l env)
                                   (eval-env r env))]
    [(let-expr x e1 e2)
     (eval-env e2 (env-add x (thunk e1 env) env))]
    [(var-expr x) (match (env-lookup x env)
                    [(thunk e1 env) (eval-env e1 env)]
                    [_ (env-lookup x env)])]
    [(if-expr eb et ef) (if (eval-env eb env)
                            (eval-env et env)
                            (eval-env ef env))]
    [(cons-expr e1 e2) (cons (thunk e1 env)
                             (thunk e2 env))]
    [(car-expr e) (match (car (eval-env e env))
                    [(thunk e1 env) (eval-env e1 env)]
                    [_ (car (eval-env e env))])]
    [(cdr-expr e) (match (cdr (eval-env e env))
                    [(thunk e1 env) (eval-env e1 env)]
                    [_ (cdr (eval-env e env))])]
    [(null-expr) null]
