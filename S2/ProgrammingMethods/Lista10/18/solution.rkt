...
(define (builtin/3 p)
  (builtin (lambda (x y) (return (p x y))) null 3))
...
(define effect-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         start-env
         `((uniform ,(effect-builtin/1 (lambda (xs) (map (lambda (x) (cons (/ 1 (length xs)) x)) xs))))
           (flip ,(effect-builtin/3 (lambda (p x y) (list (cons p x) (cons (- 1 p) y))))))))

(define (bind c k)
  (append-map (lambda (x)
                (map (lambda (y) (cons (* (car x) (car y)) (cdr y))) (k (cdr x)))) c))
...
(define (norm xs)
  (define (sum-if xs x s)
    (if (null? xs)
        s
        (if (equal? (cdar xs) x)
            (sum-if (cdr xs) x (+ s (caar xs)))
            (sum-if (cdr xs) x s))))
  (let ([ux (set->list (list->set (map (lambda (x) (cdr x)) xs)))])
    (map (lambda (x) (cons (sum-if xs x 0) x)) ux)))
    ...
    '(letrec [from-to (lambda (x n)
                      (cons x
                            (if (= x n)
                                null
                                (from-to  (+ 1 x) n))))]
       (letrec [diceN (lambda (N l)
                        (if (= N 0)
                            l
                            (diceN (- N 1) (+ l  (uniform (from-to 1 6))))))]
       (diceN (uniform (from-to 1 6)) 0))))
